...
public final class PropertyHelper {
  ...
  public static void setTargetFromProperties(final Object target,
  ...
  public static Object getProperty(final String propName, final Object target) {
    try {
      ...
      String capitalized =
          "get" + propName.substring(0, 1).toUpperCase(Locale.ENGLISH) + propName.substring(1);
      Method method = target.getClass().getMethod(capitalized);
      return method.invoke(target);
    } catch (Exception e) {
      try {
        String capitalized =
            "is" + propName.substring(0, 1).toUpperCase(Locale.ENGLISH) + propName.substring(1);
        Method method = target.getClass().getMethod(capitalized);
        return method.invoke(target);
      } catch (Exception e2) {
        return null;
  ...
  private static void setProperty(final Object target, final String propName,
    ...
    try {
      ...
      if (paramClass == int.class) {
        ...
        try {
          ...
          logger.debug("Class \"{}\" not found or could not instantiate it (Default constructor)",
              propValue.toString());
