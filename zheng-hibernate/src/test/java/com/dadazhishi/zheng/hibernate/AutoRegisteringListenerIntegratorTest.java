...
public class AutoRegisteringListenerIntegratorTest {
  ...
  public void beforeEach() {
    ...
    listeners.add(createListener(ImmutableSet.of(EventType.AUTO_FLUSH, EventType.CLEAR),
        ImmutableSet.of(RegistrationType.PREPEND)));
    listeners.add(createListener(ImmutableSet.of(EventType.DIRTY_CHECK),
        ImmutableSet.of(RegistrationType.APPEND, RegistrationType.PREPEND)));
    listeners.add(createListener(ImmutableSet.of(EventType.DELETE),
        ImmutableSet.of(RegistrationType.REPLACE)));
  ...
  public void integratorRegistersAllListeners() {
    ...
    verify(eventListenerRegistry, times(1))
        .prependListeners(EventType.AUTO_FLUSH, (AutoFlushEventListener) listeners.get(0));
    verify(eventListenerRegistry, times(1))
        .prependListeners(EventType.CLEAR, (ClearEventListener) listeners.get(0));
    verify(eventListenerRegistry, times(1))
        .prependListeners(EventType.DIRTY_CHECK, (DirtyCheckEventListener) listeners.get(1));
    verify(eventListenerRegistry, times(1))
        .appendListeners(EventType.DIRTY_CHECK, (DirtyCheckEventListener) listeners.get(1));
    verify(eventListenerRegistry, times(1))
        .setListeners(EventType.DELETE, (DeleteEventListener) listeners.get(2));
  ...
  private abstract static class TestListener
    ...
    public void onDelete(final DeleteEvent event,
        @SuppressWarnings("rawtypes") final Set transientEntities) throws HibernateException {
    }
