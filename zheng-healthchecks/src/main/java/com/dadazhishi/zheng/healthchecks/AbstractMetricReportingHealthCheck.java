 ...
 *
 * <p/>By default, gwizard-metrics creates a JmxReporter. So, if any tool is scraping the JMX
 * metrics tree, the health checks will be run at whatever rate the tools is performing JMX queries
 * that include the Metrics MBeans.
 *
 * <p/>The name of the metric will be constructed fromt he prefix read from the configuration, plus
 * the name passed to the ctor. By default, the metric mbean will have the ObjectName: {@code
...
public abstract class AbstractMetricReportingHealthCheck extends HealthCheck {
  ...
  /**
   * ctor for creating a cached gauge metric that calls a health check. If your metric might be
   * queried very often, and your health check is expensive, you might want to throttle how often
   * the check is called. <p/>NOTE: this will only serve as a throttle for the healthcheck when
   * called via metrics reporting. Any other mechanisms that might call the healthcheck will not be
   * throttled (e.g. if the HealthCheckService is configured to run periodically, or you expose the
   * healthchecks via REST)
   *
   * @see CachedGauge
   */
  public AbstractMetricReportingHealthCheck(HealthChecks healthChecks, String healthCheckName,
  ...
  /**
   * creates the Gauge/CachedGauge. uses method injection so that subclasses of this class don't
   * have to have cluttered ctor params.
   */
  @Inject
