...
/**
 * Hibernate {@code UnitOfWork} implementation which manages a ThreadLocal {@code Session} instance.
 * Invoking {@link #begin()} will open a new {@code Session} if one is not already open. Subsequent
 * invocations of {@link #get()} will return the same {@code Session} instance until {@link #end()}
 * is invoked. <br /> <br />
 *
 * This class may also be used as a {@code Provider<Session>} without the {@code UnitOfWork}
 * context. Note that any session not created during a {@link #begin()} invocation *must* be
 * manually closed. Invoking {@link #get()} and then {@link #end()} will *not* close the session
 * created from {@link #get()}.
 *
...
public class HibernateUnitOfWork implements Provider<Session>, UnitOfWork {
  ...
  public Session get() {
    ...
    if (isWorking()) {
      ...
      logger.warn(
          "Opening hibernate Session with no current UnitOfWork. This session must be manually closed.");
  ...
  public void end() {
    Preconditions.checkState(isWorking(),
        "UnitOfWork.end() invoked with no corresponding UnitOfWork.begin()");
