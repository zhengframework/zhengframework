...
/**
 * Hibernate {@link Integrator} which dynamically binds Listeners with the {@code EventType
 * EventTypes} to which they should subscribe. Any existing listener implementation may be decorated
 * with the {@link AutoRegisteringListener} in order to utilize auto-registration.
 *
...
public class AutoRegisteringListenerIntegrator implements Integrator {
  ...
  private static final Logger logger = LoggerFactory
      .getLogger(AutoRegisteringListenerIntegrator.class);
  private final ImmutableSet<? extends AutoRegisteringListener> listeners;

  @Inject
  public AutoRegisteringListenerIntegrator(
      final ImmutableSet<? extends AutoRegisteringListener> listeners) {
  ...
  private <T> void doRegister(final EventListenerRegistry registry, final EventType<T> type,
      final AutoRegisteringListener listener) {
      ...
      switch (registrationType) {
        ...
        default:
          throw new IllegalArgumentException(
              "Unknown registration type " + registrationType.toString());
      }
    }
  }

  @SuppressWarnings("unchecked")
  private <T> void replaceListener(final EventListenerRegistry registry, final EventType<T> type,
      final AutoRegisteringListener listener) {
    try {
      logger.debug(
          String.format("Registering listener %s: %s", type.eventName(), listener.getClass()));
  ...
  @SuppressWarnings("unchecked")
  private <T> void appendListener(final EventListenerRegistry registry, final EventType<T> type,
      final AutoRegisteringListener listener) {
    try {
      logger.debug(
          String.format("Registering listener %s: %s", type.eventName(), listener.getClass()));
  ...
  @SuppressWarnings("unchecked")
  private <T> void prependListener(final EventListenerRegistry registry, final EventType<T> type,
      final AutoRegisteringListener listener) {
    try {
      logger.debug(
          String.format("Registering listener %s: %s", type.eventName(), listener.getClass()));
  ...
  private void logFailure(final EventType<?> type, final AutoRegisteringListener listener) {
    logger.warn(String
        .format(
            "Failed to register event listener %s for event type %s. Listener does not implement required interface %s.",
            listener.getClass(),
            type.eventName(),
