...
public class QuerydslModule extends ConfigurationAwareModule {
  ...
  protected void configure() {
    Map<String, QuerydslConfig> configMap =
        ConfigurationBeanMapper.resolve(getConfiguration(), QuerydslConfig.class);
    for (Entry<String, QuerydslConfig> entry : configMap.entrySet()) {
      ...
      if (name.isEmpty()) {
        ...
        if (querydslConfig.isEnable()) {
          OptionalBinder.newOptionalBinder(binder(), Key.get(com.querydsl.sql.Configuration.class))
              .setDefault()
              .toProvider(() -> new com.querydsl.sql.Configuration(SQLTemplates.DEFAULT));
          bind(Key.get(SQLQueryFactory.class))
              .toProvider(
                  new SQLQueryFactoryProvider(
                      getProvider(Key.get(DataSource.class)),
                      getProvider(Key.get(com.querydsl.sql.Configuration.class))))
              .in(Singleton.class);
        }
      } else {
          ...
          bind(Key.get(SQLQueryFactory.class, named(name)))
              .toProvider(
                  new SQLQueryFactoryProvider(
                      ...
                      getProvider(Key.get(com.querydsl.sql.Configuration.class, named(name)))))
              .in(Singleton.class);
        }
      }
    }
